<div wire:key='{{$todo->id}}' class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        <div class="flex items-center">
            ...
            <input class="mr-2" wire:click='toggle({{$todo->id}})' type="checkbox" name="" id="" {{$todo->completed?"checked":""}}>
            @if ($editingTodoID === $todo->id)
            <input type="text" wire:model='editingTodoName' placeholder="Todo.."
    ...
    <div class="mt-3 text-xs text-gray-700">
        @if ($editingTodoID === $todo->id)
        <button
        class="mt-3 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600" wire:click='update'>Update</button>
    <button
        class="mt-3 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600" wire:click='cancelEdit'>Cancel</button>
        @endif
